...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const trainers = usersRows.map((user) => {
      ...
      const yearsOfExperience = faker.number.int({
        min: 1,
        max: maxExperience,
      });
      ...
      return {
        ...
        hourly_rate: faker.number
          .float({ min: 12, max: 150, precision: 0.01 })
          .toPrecision(1),
        availability: faker.helpers.arrayElement(["AVAILABLE", "UNAVAILABLE"]),
        createdAt: createdAt,
        updatedAt: faker.date.between({ from: createdAt, to: "2026-1-10" }),
      };
    });
